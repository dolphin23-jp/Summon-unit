export type DisplayMasterKind =
  'SPECIES' | 'SKILL' | 'STAGE' | 'REGION' | 'RESEARCH' | 'CATALYST' | 'REWARD' | 'ATTRIBUTE'
...
export function validateDisplayMasterEntries(
  ...
  for (const entry of entries) {
    ...
    if (entry.name.trim().length === 0)
      throw new Error(`display name must be non-empty: ${entry.id}`)
...
export function validateDisplayMasters(): DisplayMasterValidationSummary {
  const entries: DisplayMasterEntry[] = [
    ...
    ...Object.entries(RESEARCH_NAMES).map(([id, name]) => ({
      id,
      name,
      kind: 'RESEARCH' as const,
    })),
    ...Object.entries(CATALYST_NAMES).map(([id, name]) => ({
      id,
      name,
      kind: 'CATALYST' as const,
    })),
    ...Object.entries(REWARD_NAMES).map(([id, name]) => ({ id, name, kind: 'REWARD' as const })),
    ...Object.entries(ATTRIBUTE_NAMES).map(([id, name]) => ({
      id,
      name,
