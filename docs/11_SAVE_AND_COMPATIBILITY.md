...
T037デモは研究施設段階を追加するためschemaVersion 4を使用します。IndexedDB migrationは未実装ですが、メモリ内の旧入力は施設マスターがある場合に段階1へ補完します。

IndexedDB永続化とJSON import/exportは未実装です。公開済みセーブの永続変換はまだ発生しません。
