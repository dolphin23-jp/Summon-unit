...
migration保存では変換元generationを`preferredBackupGenerationId`として指定します。変換元がcurrentではなく古いバックアップであっても、そのgenerationをバックアップ先頭へ残してから上限を適用します。
...
T041はPlayerDataのschemaVersionを一段ずつ変換します。各migrationは`fromVersion`と`toVersion = fromVersion + 1`を持ち、段階飛ばし、同じfromVersionの重複、中間step欠落を拒否します。
...
- 3→4: 研究施設状態を追加
- 4→5: ステージ進行を追加
- 5→6: 地域進行マスターとの接続に対応したversion更新

migrationは入力JSONを複製して実行し、元データを変更しません。各stepの返却schemaVersionを検証し、全step完了後に`createStagePlayerData`で現行PlayerData全体を検証します。

旧schemaをロードした場合は次の順で処理します。

1. generation envelope、保存時JSONチェックサム、slot所属を検証する
2. 現行schemaまで一段ずつ変換する
3. 現行PlayerDataとして全体検証する
4. 新しいTEMPORARY generationへ書く
5. 単一transactionで新generationをcurrentへ切り替える
6. 変換元generationを最新バックアップとして保持する

変換、全体検証、仮保存検証、pointer切替のいずれかが失敗した場合、新generationをcurrentにせず旧pointerを維持します。currentが破損している場合はバックアップを新しい順に調べ、最初の検証・migration可能な世代を使用します。

## 保存操作とUI

- 起動時にlocalStorageの使用中スロットを読み、IndexedDBのcurrentを復元する
- 使用中スロットが空の場合は初期PlayerDataを保存する
- 旧schemaの場合はロード境界で自動migrationする
...
T034のデモPlayerDataは経済フィールド追加に伴いschemaVersion 2です。T035では研究状態を追加し、正式なT035以降のデータはschemaVersion 3を使用します。
